...
function Navbar(){
    ...
    return(
        <div className="mx-2 p-3 dark:text-neutral-400 w-[100%]">
            <div>
                <div className="flex items-center">
                    <button onClick={handleClickSearch}><CiSearch/></button>
                    <input type="search" ref={SearchInputRef} placeholder="Songs Albums or Podcasts" className="bg-neutral-200 dark:bg-neutral-700 text-neutral-700 dark:text-neutral-400 rounded-3xl px-5 h-10 w-80" />   
                    <div className="flex justify-right ml-8">
                        {Navitems.map((e)=>(
                            <div className={`mr-8 hover:text-yellow-400 cursor-pointer`} key={e.NavTitle} value={e.NavTitle}>
                                <h2 >{e.NavTitle}</h2>
                            </div>
                        ))}
                    </div>     
...
function MainSlider(){
    ...
    return(
        <div>
            ...
            <div className={`my-6 w-[100%] dark:text-white`}>
                <div style={{backgroundImage:`url(${Slides[currentIndex].url})`}} className={`  h-relative bg-no-repeat bg-center  rounded-2xl`}><p className={`text-white dark:text-slate-100 font-bold text-[30px] mx-10 py-20`}>{Slides[currentIndex].name}</p></div>
                {/* left arrow */}
